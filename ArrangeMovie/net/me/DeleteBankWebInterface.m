...
@implementation DeleteBankWebInterface
-(instancetype)init {
    self = [super init];
    if(self){
        self.url = [NSString stringWithFormat:@"%@%@",self.server,@"deleteBankCard.do"];
    }
    return self;
}
...
-(NSDictionary *)inboxObject:(id)param {
    ...
    if(array.count>0){
        ...
        @try {
            ...
            [dict setObject:array[2] forKey:@"bankid"];
