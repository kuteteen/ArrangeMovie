...
@implementation GetBankTypeListWebInterface
-(instancetype)init {
    self = [super init];
    if(self){
        self.url = [NSString stringWithFormat:@"%@%@",self.server,@"bankTypeList.do"];
    }
    return self;
}
...
-(id)unboxObject:(NSDictionary *)result {
    ...
    if (success==1) {
        [array addObject:@1];
        [array addObject:[result objectForKey:@"data"]];
    }else {
        [array addObject:@2];
