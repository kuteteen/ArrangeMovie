...
@interface ManegerLookCertViewController ()<SCFadeSlideViewDelegate,SCFadeSlideViewDataSource>
...
@implementation ManegerLookCertViewController
...
- (void)fetchData{
    ...
    [SCHttpOperation requestWithMethod:RequestMethodTypePost withURL:pfamdetailInterface.url withparameter:param WithReturnValeuBlock:^(id returnValue) {
        ...
        if ([result[0] intValue] == 1) {
            weakself.array = [[NSMutableArray alloc] initWithArray:result[1]];
            //刷新
...
- (void)didSelectCell:(UIView *)subView withSubViewIndex:(NSInteger)subIndex {
    ...
    photoBroseView.imagesURL = [self.array[subIndex] objectForKey:@"imgs"];
...
-(UIView *)slideView:(SCFadeSlideView *)slideView cellForPageAtIndex:(NSInteger)index {
    ...
    if(!pageView){
        ...
        [shadowImageView setRectangleBorder:[[self.array[index] objectForKey:@"imgs"] objectAtIndex:0]];
        ...
        label.text = [NSString stringWithFormat:@"%@排片详情",[self.array[index] objectForKey:@"imgdate"]];
